angular.module('ngAnimate-animate.css', ['ngAnimate'])
  ...
  .factory('animateCSSBuild', ['$window', '$timeout', function($window, $timeout) {
    ...
    var prefix = (function() {
      ...
      var styles = $window.getComputedStyle(doc.documentElement, '');
    ...
    /**
     * Get animation-duration of an element
     * @param {HTMLElement} el
     * @return {number} Returns duration in miliseconds
     */
    var getDelay = function(element) {
      var delay = $window.getComputedStyle(element)[prefix.js + 'AnimationDuration'];

      // Convert string to number, convert s to ms
    ...
    return function(baseClass, classNames) {
      ...
      var animateCSSStart = function(element, className, delay, done) {
        ...
        delay = delay || getDelay(element[0]) || 2000;
        var timer = $timeout(done, delay, false);
